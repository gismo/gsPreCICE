...
int main(int argc, char *argv[])
{
    ...
    index_t remeshStep = 25; // Step at which to remesh
    bool enableRemesh = false;
    ...
    cmd.addInt("", "remeshStep", "Time step at which to perform remeshing", remeshStep);
    cmd.addSwitch("", "enableRemesh", "Enable mesh refinement and remeshing", enableRemesh);
...
// ----------------------------------------------------------------------------------------------
    ...
    // Use pointer for lookup function to allow reassignment
    std::unique_ptr<gsLookupFunction<real_t>> g_L_ptr(new gsLookupFunction<real_t>(quad_xy,quad_stress));
    // gsPreCICEFunction<real_t> g_C(&participant,SolidMesh,StressData,patches,patches.geoDim(),false);
    // Add all BCs
    // Coupling interface
    bcInfo.addCondition(0, boundary::north,  condition_type::neumann , g_L_ptr.get(),-1,true);
    bcInfo.addCondition(0, boundary::east,  condition_type::neumann  , g_L_ptr.get(),-1,true);
    bcInfo.addCondition(0, boundary::west,  condition_type::neumann  , g_L_ptr.get(),-1,true);
...
// ----------------------------------------------------------------------------------------------
    ...
    gsParaviewCollection meshCollection("./output/mesh");
    ...
    if (plot)
    {
        ...
        gsWriteParaview<>(solField, fileName, 500, true); // true to also save mesh
        fileName = "solution" + util::to_string(timestep) + "0";
        collection.addTimestep(fileName,time,".vts");
        meshCollection.addTimestep(fileName + "_mesh",time,".vtp");
    }
    ...
    while (participant.isCouplingOngoing())
    {
        ...
        // Only read data if we haven't just remeshed
        if (!justRemeshed)
        {
        ...
        else
        {
            ...
            // Check if it's time to remesh
            if (enableRemesh && timestep == remeshStep)
            {
                gsInfo << "\n===== REMESHING AT TIMESTEP " << timestep << " =====\n";
                
                // Store current solution coefficients
                gsMatrix<> currentCoefs = solution.patch(0).coefs();
                
                // Reset the mesh in preCICE
                participant.resetMesh(SolidMesh);
                
                // Refine the basis and geometry
                bases.uniformRefine();
                patches.uniformRefine();
                
                gsInfo << "Mesh refined. New basis size: " << bases.basis(0).size() << "\n";
                
                // Recompute quadrature points on refined mesh
                quad_uv = gsQuadrature::getAllNodes(bases.basis(0), quadOptions, couplingInterfaces);
                quad_xy = patches.patch(0).eval(quad_uv);
                
                // Re-register the mesh with preCICE
                participant.addMesh(SolidMesh, quad_xy, quad_xyIDs);
                
                // Update quad_stress size for new mesh
                quad_stress.resize(2, quad_xy.cols());
                quad_stress.setZero();
                
                // Update the lookup function with new data
                g_L_ptr.reset(new gsLookupFunction<real_t>(quad_xy, quad_stress));
                
                // Update boundary conditions with new lookup function
                bcInfo.clear();
                bcInfo.addCondition(0, boundary::north,  condition_type::neumann , g_L_ptr.get(),-1,true);
                bcInfo.addCondition(0, boundary::east,  condition_type::neumann  , g_L_ptr.get(),-1,true);
                bcInfo.addCondition(0, boundary::west,  condition_type::neumann  , g_L_ptr.get(),-1,true);
                ...
                // Rebuild assemblers with new basis
                // Re-create mass assembler with refined basis
                massAssembler = gsMassAssembler<real_t>(patches, bases, bcInfo, gravity);
                massAssembler.options().setReal("Density", rho);
                massAssembler.assemble();
                M = massAssembler.matrix();
                
                // Re-create elasticity assembler with refined basis
                assembler = gsElasticityAssembler<real_t>(patches, bases, bcInfo, g);
                assembler.options().setReal("YoungsModulus", E);
                assembler.options().setReal("PoissonsRatio", nu);
                assembler.options().setInt("MaterialLaw", material_law::saint_venant_kirchhoff);
                assembler.assemble();
                ...
                gsMatrix<> newSolVector;
                newSolVector.setZero(assembler.numDofs(), 1);
                
                // Simple projection: evaluate old solution at new DOFs
                // (More sophisticated projection methods could be used)
                gsMultiPatch<> oldSolution = solution;
                index_t newDofs = assembler.numDofs();
                
                // Update solution vectors size
                U.conservativeResize(newDofs);
                V.conservativeResize(newDofs);
                A.conservativeResize(newDofs);
                U_checkpoint.conservativeResize(newDofs);
                V_checkpoint.conservativeResize(newDofs);
                A_checkpoint.conservativeResize(newDofs);
                
                // Zero out new DOFs (simple approach)
                for (index_t i = solVector.size(); i < newDofs; ++i)
                {
                    U(i) = 0;
                    V(i) = 0;
                    A(i) = 0;
                ...
                // Rebuild time integrator with new system size
                delete timeIntegrator;
                C.resize(assembler.numDofs(), assembler.numDofs());
                ...
                // Save mesh immediately after remeshing
                if (plot)
                {
                    // Save solution with mesh after remeshing
                    std::string fileName = "./output/solution_remeshed_" + util::to_string(timestep);
                    gsMultiPatch<> zeroSolution = solution;
                    zeroSolution.patch(0).coefs().setZero(); // Use zero solution to clearly see mesh
                    gsField<> meshField(patches, zeroSolution);
                    gsWriteParaview<>(meshField, fileName, 500, true); // true to also save mesh
                    fileName = "solution_remeshed_" + util::to_string(timestep) + "0";
                    meshCollection.addTimestep(fileName + "_mesh",time,".vtp");
                    gsInfo << "Saved remeshed mesh to " << fileName << "_mesh.vtp\n";
                }
                
                gsInfo << "===== REMESHING COMPLETED =====\n\n";
                
                // Set flag to skip data reading in next iteration
                justRemeshed = true;
            }
